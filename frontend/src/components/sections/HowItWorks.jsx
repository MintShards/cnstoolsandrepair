export default function HowItWorks({
  ...
  return (
    <section className={`px-6 sm:px-8 lg:px-12 py-16 sm:py-20 lg:py-24 ${backgroundColor}`}>
      <div className="max-w-screen-xl mx-auto">
        <div className="text-center mb-8 sm:mb-10 lg:mb-16">
          <p className="text-red-700 dark:text-accent-orange text-[10px] sm:text-xs font-black uppercase tracking-[0.20em] sm:tracking-[0.25em] mb-2">{content.label}</p>
          <h2 className="text-2xl sm:text-3xl lg:text-4xl font-black tracking-tight uppercase px-4">{content.heading}</h2>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 sm:gap-8 lg:gap-10">
        {content.steps
          ...
          <div key={step.number} className="flex lg:flex-col gap-4 sm:gap-5 lg:gap-4 items-start lg:items-center lg:text-center">
            ...
            <div>
              <h3 className="text-base sm:text-lg lg:text-xl font-black mb-1 sm:mb-1.5 lg:mb-2 uppercase tracking-tight">{step.title}</h3>
              <p className="text-xs sm:text-sm lg:text-base text-slate-600 dark:text-slate-400 leading-relaxed">{step.description}</p>
            </div>
          </div>
        ))}
        </div>
