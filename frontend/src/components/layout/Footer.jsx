    ...
    <footer className="p-8 pb-32 lg:pb-12 bg-white dark:bg-slate-950 border-t border-slate-200 dark:border-slate-800">
      <div className="max-w-screen-xl mx-auto">
              ...
              <ul className="flex flex-col gap-3 text-sm font-bold">
                <li className="flex items-start gap-2">
                  ...
                  <div className="text-slate-600 dark:text-slate-300">
                    <p>{settings?.contact?.address?.street || 'Unit 65 13335 115 Ave'}</p>
                    <p>{settings?.contact?.address?.city || 'Surrey'}, {settings?.contact?.address?.province || 'BC'} {settings?.contact?.address?.postalCode || 'V3R 0R8'}</p>
                    <p>Canada</p>
                  </div>
                </li>
                <li className="flex items-center gap-2">
                  <span className="material-symbols-outlined text-base text-primary">mail</span>
                  <a href={`mailto:${settings?.contact?.email || 'contact@cnstoolsandrepair.com'}`} className="hover:text-primary transition-colors text-slate-600 dark:text-slate-300">
                    {settings?.contact?.email || 'contact@cnstoolsandrepair.com'}
                  </a>
                </li>
                <li className="flex items-center gap-2">
                  <span className="material-symbols-outlined text-base text-primary">schedule</span>
                  <span className="text-slate-600 dark:text-slate-300">{settings?.hours?.weekdays || 'Monday - Friday: 9:00 AM - 4:00 PM'}</span>
                </li>
              </ul>
            </div>
          </div>
        ...
        <div className="pt-8 mt-8 border-t border-slate-200 dark:border-slate-800">
          {/* Social Media Icons */}
          {settings?.social && (settings.social.facebook || settings.social.linkedin || settings.social.instagram) && (
            <div className="flex justify-center gap-4 mb-6">
              {settings.social.facebook && (
                <a
                  ...
                  className="flex items-center justify-center w-10 h-10 rounded-full bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 hover:bg-primary hover:text-white dark:hover:bg-primary dark:hover:text-white transition-colors"
                  aria-label="Facebook"
                >
                  <FacebookIcon />
                </a>
          ...
          <p className="text-center text-[10px] text-slate-400 font-black uppercase tracking-widest">
            © 2024 CNS TOOLS AND REPAIR. All Rights Reserved.
          </p>
        </div>
      </div>
    </footer>
