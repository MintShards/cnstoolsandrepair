...
export default function RepairRequestsTab({ onConvertSuccess, onCountUpdate }) {
  ...
  const getStatusBadge = (status) => {
    const statusConfig = {
      pending:   { bg: 'bg-yellow-100 dark:bg-yellow-900/30', text: 'text-yellow-800 dark:text-yellow-300', border: 'border-yellow-400 dark:border-yellow-700/60', dot: 'bg-yellow-500 dark:bg-yellow-400', label: 'Pending' },
      converted: { bg: 'bg-cyan-100 dark:bg-cyan-900/30',    text: 'text-cyan-800 dark:text-cyan-300',    border: 'border-cyan-400 dark:border-cyan-700/60',   dot: 'bg-cyan-500 dark:bg-cyan-400',   label: 'Converted to WO' },
    };
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="text-2xl font-black text-slate-900 dark:text-white uppercase tracking-tight">Online Repair Requests</h2>
      ...
      <div className="mb-5 flex flex-wrap items-center gap-3">
        <div className="relative flex-1 min-w-[220px]">
          ...
          <input
            ...
            className="w-full pl-9 pr-8 py-2 bg-slate-50 dark:bg-slate-800/80 border border-slate-300 dark:border-slate-700 rounded-xl text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 text-sm focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary/30 transition-all"
          />
          {searchQuery && (
            <button
              onClick={() => setSearchQuery('')}
              className="absolute right-2.5 top-1/2 -translate-y-1/2 text-slate-500 hover:text-slate-600 dark:hover:text-slate-300 transition-colors"
            >
      ...
      <div className="bg-slate-100 dark:bg-slate-800/60 rounded-xl border border-slate-200 dark:border-slate-700/60 shadow-lg shadow-black/5 dark:shadow-black/20 overflow-hidden">
        ...
        <div className="px-5 py-3.5 border-b border-slate-200 dark:border-slate-700/60 bg-slate-50 dark:bg-slate-800/40 flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            ...
            <span className="text-sm font-bold text-slate-900 dark:text-white">Repair Requests</span>
            {!loading && (
              <span className="text-xs font-black px-2 py-0.5 rounded-full bg-slate-200/60 dark:bg-slate-700/60 text-slate-600 dark:text-slate-300 border border-slate-300 dark:border-slate-600/50">
                {totalResults}
              </span>
            )}
          </div>
        ...
        {loading ? (
          <div className="text-center py-12">
            ...
            <p className="mt-2 text-slate-500 dark:text-slate-400 text-sm">Loading repair requests…</p>
          </div>
        ) : filteredQuotes.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 px-6">
            <div className="w-16 h-16 bg-slate-200/40 dark:bg-slate-700/40 rounded-2xl flex items-center justify-center mb-4">
          ...
          <div className="bg-white dark:bg-slate-800 rounded-2xl max-w-5xl w-full my-8 border border-slate-200/50 dark:border-slate-700/50 shadow-2xl shadow-black/10 dark:shadow-black/50 animate-fadeInScale overflow-hidden" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="sticky top-0 bg-white/95 dark:bg-slate-800/95 backdrop-blur-sm border-b border-slate-200 dark:border-slate-700/60 px-6 py-4 flex items-center justify-between z-10">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-lg font-black text-slate-900 dark:text-white">Repair Request <span className="text-primary font-mono">{selectedQuote.request_number}</span></h3>
            ...
            <div className="p-6 space-y-6">
              ...
              <div>
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <h4 className="text-xs font-bold text-slate-600 dark:text-slate-300 uppercase tracking-wide">Tools ({selectedQuote.tools.length})</h4>
                </div>
                <div className="space-y-3">
                  {selectedQuote.tools.map((tool, idx) => (
                    <div key={idx} className="bg-slate-50 dark:bg-slate-900/60 rounded-xl border border-slate-200 dark:border-slate-700/60 overflow-hidden shadow-sm">
                      {/* Tool header */}
                      <div className="p-4 bg-slate-50 dark:bg-slate-800/40 border-b border-slate-200 dark:border-slate-700/60">
                        <div className="flex items-start justify-between gap-3">
                          <div className="flex items-start gap-3">
                            ...
                            <div>
                              <div className="font-bold text-slate-900 dark:text-white text-sm">{tool.tool_brand} {tool.tool_model}</div>
                          ...
                          {tool.quantity === 1 ? null : (
                            <span className="text-xs px-2 py-0.5 rounded-full bg-slate-200/60 dark:bg-slate-700/60 border border-slate-300 dark:border-slate-600/50 text-slate-500 dark:text-slate-400 font-bold flex-shrink-0">×{tool.quantity}</span>
                          )}
                        </div>
                      </div>
                      {/* Problem description */}
                      <div className="px-4 py-3">
                        ...
                        <p className="text-sm text-slate-600 dark:text-slate-300 leading-relaxed">{tool.problem_description}</p>
                      </div>
                    </div>
                  ))}
                </div>
                    ...
                    {selectedQuote.photos.map((photo, idx) => (
                      <div key={idx} className="aspect-square cursor-pointer group relative rounded-xl overflow-hidden border border-slate-200 dark:border-slate-700/60 hover:border-primary/50 transition-all" onClick={() => setSelectedPhoto(photo)}>
                        ...
                        <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                          <span className="material-symbols-outlined text-slate-900 dark:text-white text-2xl">zoom_in</span>
                        </div>
                      </div>
                    ))}
                  </div>
      ...
      {/* Photo Lightbox */}
      {selectedPhoto && (
        <div className="fixed inset-0 z-[60] bg-black/90 dark:bg-black/95 flex items-center justify-center p-4" onClick={() => setSelectedPhoto(null)}>
          <button className="absolute top-4 right-4 w-10 h-10 flex items-center justify-center rounded-xl bg-slate-50 dark:bg-slate-800/80 text-slate-900 dark:text-white hover:text-slate-600 dark:hover:text-slate-300 transition-colors" onClick={() => setSelectedPhoto(null)}>
            <span className="material-symbols-outlined text-xl">close</span>
          </button>
          <img
          ...
          <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200/50 dark:border-slate-700/50 max-w-md w-full shadow-2xl shadow-black/10 dark:shadow-black/40 animate-[fadeInScale_0.2s_ease-out] overflow-hidden" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="p-6">
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  <h3 className="text-base font-black text-slate-900 dark:text-white uppercase tracking-tight">Convert to Work Order</h3>
                  <p className="text-xs text-slate-500 mt-0.5">This will create a tracked repair job</p>
                </div>
                <button
                  onClick={() => setConvertConfirm(null)}
                  className="ml-auto w-8 h-8 flex items-center justify-center rounded-lg bg-slate-200/60 dark:bg-slate-700/60 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white transition-all"
                >
              ...
              <div className="space-y-3 mb-5">
                <p className="text-slate-600 dark:text-slate-300 text-sm">
                  Convert <span className="font-mono font-bold text-slate-900 dark:text-white">{convertConfirm.request_number}</span> into a tracked Work Order?
                </p>
                <div className="bg-slate-50 dark:bg-slate-900/60 rounded-xl border border-slate-200 dark:border-slate-700/60 p-3 text-xs space-y-1.5">
                  <div className="flex gap-2"><span className="text-slate-500 w-16">Customer</span><span className="text-slate-600 dark:text-slate-300 font-medium">{convertConfirm.company_name || `${convertConfirm.first_name} ${convertConfirm.last_name}`}</span></div>
                  <div className="flex gap-2"><span className="text-slate-500 w-16">Tools</span><span className="text-slate-600 dark:text-slate-300">{convertConfirm.tools.length} tool{convertConfirm.tools.length !== 1 ? 's' : ''}</span></div>
                </div>
              ...
              <div className="flex gap-3">
                <button onClick={() => setConvertConfirm(null)} className="flex-1 px-4 py-2.5 bg-slate-200/60 dark:bg-slate-700/60 hover:bg-slate-200 dark:hover:bg-slate-700 border border-slate-300 dark:border-slate-600/50 text-slate-900 dark:text-white rounded-xl font-bold text-sm transition-all">
                  Cancel
                </button>
                <button
                  onClick={handleConvertConfirm}
                  disabled={!!convertingId}
                  className="flex-1 px-4 py-2.5 bg-orange-500 hover:bg-orange-600 dark:bg-orange-900/50 dark:hover:bg-orange-900/70 border border-orange-400 dark:border-orange-700/50 text-white dark:text-orange-200 rounded-xl font-bold text-sm transition-all disabled:opacity-50 flex items-center justify-center gap-2"
                >
                  <span className="material-symbols-outlined text-sm">build_circle</span>
          ...
          <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200/50 dark:border-slate-700/50 max-w-md w-full shadow-2xl shadow-black/10 dark:shadow-black/40 animate-[fadeInScale_0.2s_ease-out] overflow-hidden" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="p-6">
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  <h3 className="text-base font-black text-slate-900 dark:text-white uppercase tracking-tight">Delete Request</h3>
              ...
              <div className="space-y-3 mb-5">
                <p className="text-slate-600 dark:text-slate-300 text-sm">
                  Permanently delete <span className="font-mono font-bold text-slate-900 dark:text-white">{deleteConfirmId.request_number}</span>?
                </p>
                <div className="bg-slate-50 dark:bg-slate-900/60 rounded-xl border border-slate-200 dark:border-slate-700/60 p-3 text-xs space-y-1.5">
                  <div className="flex gap-2"><span className="text-slate-500 w-16">Customer</span><span className="text-slate-600 dark:text-slate-300 font-medium">{deleteConfirmId.company_name || `${deleteConfirmId.first_name} ${deleteConfirmId.last_name}`}</span></div>
                  <div className="flex gap-2"><span className="text-slate-500 w-16">Tools</span><span className="text-slate-600 dark:text-slate-300">{deleteConfirmId.tools.length}</span></div>
                  <div className="flex gap-2"><span className="text-slate-500 w-16">Photos</span><span className="text-slate-600 dark:text-slate-300">{deleteConfirmId.photos.length}</span></div>
                </div>
                <p className="text-red-600 dark:text-red-400/80 text-xs font-bold flex items-center gap-1.5">
                  <span className="material-symbols-outlined text-sm">warning</span>
                  All associated photos will be permanently deleted from storage.
                </p>
              </div>
              <div className="flex gap-3">
                <button onClick={() => setDeleteConfirmId(null)} className="flex-1 px-4 py-2.5 bg-slate-200/60 dark:bg-slate-700/60 hover:bg-slate-200 dark:hover:bg-slate-700 border border-slate-300 dark:border-slate-600/50 text-slate-900 dark:text-white rounded-xl font-bold text-sm transition-all">
                  Cancel
                </button>
                <button onClick={handleDeleteConfirm} className="flex-1 px-4 py-2.5 bg-red-600 hover:bg-red-700 dark:bg-red-900/60 dark:hover:bg-red-800/80 border border-red-500 dark:border-red-700/50 text-white dark:text-red-200 rounded-xl font-bold text-sm transition-all flex items-center justify-center gap-2">
